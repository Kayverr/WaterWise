...
export function normalizeBillingRecord(record) {
  ...
  return {
    ...
    purok:
      record.consumer?.purok ??
      (record.consumers?.purok_no != null ? `Purok ${record.consumers.purok_no}` : record.purok) ??
      "Unassigned",
    ...
    address: record.consumer?.purok ?? (record.consumers?.purok_no != null ? `Purok ${record.consumers.purok_no}` : record.address) ?? "Address not available",
