...
export default function PaymentProcessingPage() {
  ...
  const recordPayment = async (payment) => {
    ...
    try {
      setError("");
      setSuccess("");
      const updatedBilling = await recordPaymentRequest({
      ...
      const savedPayment = {
        ...
        remainingBalance: Number(updatedBilling.remaining_balance ?? payment.remainingBalance),
        paymentStatus: updatedBilling.status ?? payment.paymentStatus,
      ...
      setSuccess(`Payment ${updatedBilling.payment_slot ?? ""} for ${payment.consumerName} was recorded in the database successfully.`);
      return true;
    } catch (requestError) {
